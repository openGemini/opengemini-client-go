...
func TestWriteBatchLineProtocol(t *testing.T) {
	c := testDefaultClient(t)
	...
	err = c.WriteBatchLineProtocol(context.Background(), database, lineProtocol)
	assert.Nil(t, err)
...
func TestWriteBatchLineProtocolWithRp(t *testing.T) {
	c := testDefaultClient(t)
	...
	testMeasurement := randomMeasurement()
	lineProtocol := testMeasurement + ",tag=tag1 field=0.1 " + fmt.Sprintf("%v", time.Now().UnixNano()) + "\n"
	err = c.WriteBatchLineProtocolWithRp(context.Background(), database, "testRp", lineProtocol)
	assert.Nil(t, err)
